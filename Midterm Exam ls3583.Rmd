...
library(tidyverse)
set.seed(100)
knn.mis.list <- function( k.values ){
  mis.list <- list()
  mis.k.df <- data.frame()
  for (i in 1:5){
    ...
    for (k in 1:length(k.values)){
     ...
     mis.k.df[k, 'k'] <-k
     mis.k.df[k,'mis_rate'] <- mis.k
    }
  mis.list[[i]] <- mis.k.df
  }
  DT <- rbindlist(mis.list)
  DT <- DT %>%group_by(k) %>% summarise(mis.rate.average = mean(mis_rate))
  return(DT)
}


knn.mis.list(k.values = 1:20)
