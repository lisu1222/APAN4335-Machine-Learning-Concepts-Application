...
set.seed(123) 
x <- model.matrix(readmitted ~., train)[,-6]
y <- train$readmitted
# Fit the model on the training data
fit.lasso.one <- glmnet(x, y, alpha = 1, family = "binomial")
# Make predictions on the test data
x.test <- model.matrix(readmitted ~., test)[,-6]
prob.lasso.one <- predict(fit.lasso.one, newx = x.test)
pred.lasso.one <- ifelse(prod.lasso.one > 0.5, 1, 0)
# Model accuracy
...
pred.lasso.many <- function(){
  pred.list <- list()
  for (i in 1:5){
    x <- model.matrix(readmitted ~ race + gender + time_in_hospital + num_medications, train.subset[[i]])[,-6]
    y <- train.subset[[i]]$readmitted
    lasso.many <- glmnet(x, y, alpha = 1, family = "binomial")
    #make predictions on the test data
    x.test <- model.matrix(readmitted ~., test.subset[[i]])[,-6]
    pred.list[[i]] <- data.frame(ifelse((predict(fit.lasso.one, newx = x.test)) >0.5,1,0))
  }
  return (pred.list)
}
...
# Fit the cv model on the training set
