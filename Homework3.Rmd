...
subdat <- scale(measured.dat[predictors])
```
...
data.frame(Actual = round(measured.dat[1:10,"survived"],2), Pred = round(pred.glm.10,2) )
...
##being assigned to hcluster.group2 , hcluster.group3, and hcluster.group4, all increases the likelihood of survival, compared to being assigned to hcluster.group1. While being assigned to group5 decreases the likelihood of survival compared to that in group1.
#but only group2 and group3 assignments add information to the prediction, using a 0.05 level of statistical significance. Being assigned to group2 increases the log odds of survival by 3.507, and being assigned to group3 increases the log odds of survival by 2.092.
